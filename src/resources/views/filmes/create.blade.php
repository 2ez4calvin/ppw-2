    ...
    <div class="p-4 caixa">
        <form action="{{ route('filmes.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select id="studio_id" name="studio_id" class="form-control input-custom @error('studio_id') is-invalid @enderror" required>
                        ...
                        @foreach($studios as $studio)
                            <option value="{{ $studio->id }}" {{ old('studio_id') == $studio->id ? 'selected' : '' }}>{{ $studio->nome }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3">
                ...
                <textarea id="sinopse" name="sinopse" rows="4" class="form-control input-custom @error('sinopse') is-invalid @enderror">{{ old('sinopse') }}</textarea>
                @error('sinopse') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="mb-3">
                <label for="genres" class="form-label fw-semibold">Gêneros</label>
                <select id="genres" name="genres[]" class="form-control input-custom @error('genres') is-invalid @enderror @error('genres.*') is-invalid @enderror" multiple required style="height: 100px;">
            ...
            <div class="mb-3">
                ...
                <select id="directors" name="directors[]" class="form-control input-custom @error('directors') is-invalid @enderror" multiple required style="height: 80px;">
                    @foreach($directors as $director)
                        <option value="{{ $director->id }}" {{ is_array(old('directors')) && in_array($director->id, old('directors')) ? 'selected' : '' }}>{{ $director->person->nome }}</option>
                    @endforeach
                </select>
                @error('directors') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="actors" name="actors[]" class="form-control input-custom" multiple style="height: 100px;">
                    @foreach($actors as $actor)
                        <option value="{{ $actor->id }}" {{ is_array(old('actors')) && in_array($actor->id, old('actors')) ? 'selected' : '' }}>{{ $actor->person->nome }}</option>
                    @endforeach
                </select>
            </div>

            <div class="mb-3">
                <label for="writers" class="form-label fw-semibold">Escritores / Roteiristas</label>
                <select id="writers" name="writers[]" class="form-control input-custom" multiple style="height: 80px;">
                    @foreach($writers as $writer)
                        <option value="{{ $writer->id }}" {{ is_array(old('writers')) && in_array($writer->id, old('writers')) ? 'selected' : '' }}>{{ $writer->person->nome }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <select id="producers" name="producers[]" class="form-control input-custom" multiple style="height: 80px;">
                    @foreach($producers as $producer)
                        <option value="{{ $producer->id }}" {{ is_array(old('producers')) && in_array($producer->id, old('producers')) ? 'selected' : '' }}>{{ $producer->person->nome }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                @error('imagens.*') <div class="invalid-feedback">Cada arquivo deve ser uma imagem válida (jpg, png, webp) de até 2MB.</div> @enderror
            </div>
