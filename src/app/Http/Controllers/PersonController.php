    ...
    try {
        DB::transaction(function () use ($request, $person, &$arquivosNovosSalvos) {
            ...
            if (in_array('ator', $funcoesSelecionadas)) {
                if (!$person->actor) { $person->actor()->create(); }
            } else {
                if ($person->actor) { $person->actor()->delete(); }
            }

            if (in_array('diretor', $funcoesSelecionadas)) {
                if (!$person->director) { $person->director()->create(); }
            } else {
                if ($person->director) { $person->director()->delete(); }
            }

            if (in_array('escritor', $funcoesSelecionadas)) {
                if (!$person->writer) { $person->writer()->create(); }
            } else {
                if ($person->writer) { $person->writer()->delete(); }
            }

            if (in_array('produtor', $funcoesSelecionadas)) {
                if (!$person->producer) { $person->producer()->create(); }
            } else {
                if ($person->producer) { $person->producer()->delete(); }
            }
